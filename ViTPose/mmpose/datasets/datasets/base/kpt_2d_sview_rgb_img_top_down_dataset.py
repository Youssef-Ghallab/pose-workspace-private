...
class Kpt2dSviewRgbImgTopDownDataset(Dataset, metaclass=ABCMeta):
    ...
    def __init__(self,
        ...
        self.bbox_center_jitter_prob = data_cfg.get(
            'bbox_center_jitter_prob', 0.3)
        self.bbox_center_jitter_factor = data_cfg.get(
            'bbox_center_jitter_factor', 0.4)
    ...
    def _xywh2cs(self, x, y, w, h, padding=1.25):
        ...
        if ((not self.test_mode)
                and np.random.rand() < self.bbox_center_jitter_prob):
            center += self.bbox_center_jitter_factor * (
                np.random.rand(2) - 0.5) * [w, h]
    ...
    def prepare_data(self, idx, attach_dataset=False):
        """Prepare the raw sample dict before pipeline transforms."""
        results = copy.deepcopy(self.db[idx])
        results['ann_info'] = self.ann_info
        if attach_dataset:
            results['_dataset'] = self
            results['_sample_idx'] = idx
        return results

    def get_raw_sample(self, idx):
        """Expose a clean raw sample for pipeline transforms."""
        return self.prepare_data(idx, attach_dataset=False)

    def __getitem__(self, idx):
        """Get the sample given index."""
        results = self.prepare_data(idx, attach_dataset=True)
